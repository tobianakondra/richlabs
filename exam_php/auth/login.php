...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // On cherche l'utilisateur par son email OU son username
    $sql = "SELECT id, username, email, password, nom, prenom FROM users WHERE email = ? OR username = ?";
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (mysqli_num_rows($result) == 1) {
                ...
                if (password_verify($password, $row["password"])) {
                    ...
                    $_SESSION["id"] = $row["id"]; // IMPORTANT : On stocke l'ID ici !
                    $_SESSION["username"] = $row["username"];
                    $_SESSION["email"] = $row["email"];
                    $_SESSION["nom_complet"] = $row["prenom"] . " " . $row["nom"];
                ...
                $login_err = "Aucun compte trouvé avec cet identifiant.";
            }
        }
